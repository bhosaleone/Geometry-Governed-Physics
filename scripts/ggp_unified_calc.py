...
class GGPCalculator:
    ...
    def audit_constants(self):
        ...
        print(f"Channel Cost (alpha/4pi): {channel_cost:.4e}")
        print(f"Max Debt Ceiling (n_max): {n_max:.4f}")
        print(f"Identity Check (n_max * alpha / 8pi^2): {n_max * self.alpha / (8 * math.pi**2):.1f}")
    ...
    def audit_mass_debt(self):
        ...
        print(f"Proton (lambda={lp:.3f}, n=4):")
    ...
    def audit_photon_ground(self):
        ...
        l_photon = 0.0
        m_zero = ((self.lambda_star - 0) / (self.lambda_star - 20))**9.381966
        print(f"Ground-State Multiplier (m(0)/m_e): {m_zero:.2e}")
        print("Note: The photon represents the zero-energy limit where the debt loop collapses.")

    def audit_c60_spectroscopy(self):
        print("\n--- LAYER 4: C60 SPECTROSCOPY RATIO ---")
        freq_ratio_obs = 1183 / 576
        mode_ratio_pred = 6.0 / 3.0 # S5 K-modes
        ...
        print(f"Spectral Match Error:       {error*100:.2f}%")

    def run(self):
        print("="*65)
        print("GGP UNIFIED CALCULATION SUITE: CORRECTED HADRONIC SCALE".center(65))
        print("="*65 + "\n")
        self.audit_constants()
        self.audit_mass_debt()
        self.audit_photon_ground()
        self.audit_c60_spectroscopy()
        print("\n" + "="*65)
